...
- [packages/server-core/src/transport/__tests__/error-codes.test.ts](file://packages/server-core/src/transport/__tests__/error-codes.test.ts)
</cite>

## 更新摘要
**所做更改**
- 新增浏览器能力系统章节，详细介绍 client:browser:invoke 能力的实现
- 增强错误代码处理机制，添加新的错误码定义和处理逻辑
- 更新能力机制说明，涵盖客户端能力声明和验证流程
- 添加浏览器工具集成的完整工作流程说明
...
- 传输层安全机制：TLS 配置与认证握手
- 性能优化策略：消息队列与背压处理
- **新增** 浏览器能力系统：client:browser:invoke 能力的完整实现
- **增强** 错误代码处理：新的错误码定义与处理机制
- 面向网络通信与服务器开发人员的实践建议
...
- **新增** 能力定义：packages/server-core/src/transport/capabilities.ts
- **新增** 浏览器能力协议：packages/server-core/src/transport/browser-capability.ts
- **新增** 浏览器能力实现：apps/electron/src/main/browser-pane-manager.ts
...
BBridge["main/browser-pane-manager.ts"]
end
subgraph "共享协议"
...
subgraph "服务器核心"
PCapabilities["server-core/transport/capabilities.ts"]
PBrowserCap["server-core/transport/browser-capability.ts"]
end
subgraph "外部"
...
Tests --> TClient
Wait --> TClient
BBridge --> PCapabilities
BBridge --> PBrowserCap
PCapabilities --> PBrowserCap
```
...
- [packages/server-core/src/transport/capabilities.ts:1-37](file://packages/server-core/src/transport/capabilities.ts#L1-L37)
- [packages/server-core/src/transport/browser-capability.ts:1-54](file://packages/server-core/src/transport/browser-capability.ts#L1-L54)
- [apps/electron/src/main/browser-pane-manager.ts:2386-2737](file://apps/electron/src/main/browser-pane-manager.ts#L2386-L2737)
...
- **新增** 能力系统
  - 客户端能力声明：通过 clientCapabilities 字段声明支持的能力列表
  - 能力验证：服务器端验证客户端是否具备相应能力
  - 能力路由：根据能力选择合适的客户端进行处理
...
客户端与服务器通过 WebSocket 连接承载该信封，服务器维护多路客户端，客户端负责自动重连与工作区切换路由。**新增** 的能力系统通过 clientCapabilities 字段实现客户端能力声明和验证。
...
C->>S : "握手handshake<br/>包含clientCapabilities"
S-->>C : "握手确认handshake_ack<br/>包含registeredChannels"
...
### 能力定义与声明
**新增** 传输层引入了完整的客户端能力系统，通过 clientCapabilities 字段实现：

- **能力声明**：客户端在握手时通过 clientCapabilities 字段声明支持的能力列表
- **能力枚举**：定义了标准的能力名称常量，如 CLIENT_BROWSER_INVOKE、CLIENT_OPEN_EXTERNAL 等
- **本地能力**：LOCAL_CLIENT_CAPABILITIES 包含所有本地 Electron 客户端可提供的能力

```mermaid
graph LR
subgraph "能力系统"
A["clientCapabilities<br/>握手字段"]
B["CLIENT_BROWSER_INVOKE<br/>浏览器能力"]
C["LOCAL_CLIENT_CAPABILITIES<br/>本地能力列表"]
D["能力验证<br/>hasClientCapability"]
E["能力路由<br/>findClientsWithCapability"]
end
A --> B
B --> C
B --> D
...
**图表来源**
- [packages/server-core/src/transport/capabilities.ts:25-36](file://packages/server-core/src/transport/capabilities.ts#L25-L36)
- [packages/server-core/src/transport/capabilities.ts:28-36](file://packages/server-core/src/transport/capabilities.ts#L28-L36)

### client:browser:invoke 能力实现
**新增** client:browser:invoke 能力提供了远程代理桌面浏览器功能：

- **协议定义**：BrowserCapabilityRequest 定义了浏览器操作的请求格式
- **方法映射**：支持 IBrowserPaneManager 的完整方法集，包括生命周期、导航、交互等操作
- **版本控制**：BROWSER_CAPABILITY_VERSION 确保协议兼容性
- **参数传递**：通过 positional args 传递方法参数，保持调用一致性

浏览器能力支持的方法包括：
- **生命周期管理**：createForSession、getOrCreateForSession、destroyInstance 等
- **导航操作**：navigate、goBack、goForward 等
- **用户交互**：clickElement、fillElement、typeText、evaluate 等
- **系统集成**：setClipboard、getClipboard、screenshot 等

### 能力验证与路由机制
**新增** 服务器端对客户端能力进行严格验证：

- **能力检查**：RemoteBrowserPaneManager 在调用前验证客户端是否具备 client:browser:invoke 能力
- **错误处理**：当客户端不具备能力时抛出 CodedError，包含具体错误码和描述
- **客户端选择**：根据能力筛选合适的客户端进行处理
- **降级处理**：当首选客户端不可用时，自动寻找其他具备能力的客户端

```mermaid
sequenceDiagram
participant S as "服务器"
participant RM as "RemoteBrowserPaneManager"
participant C as "客户端"
participant BM as "BrowserPaneManager"
S->>RM : "请求浏览器操作"
RM->>RM : "检查客户端能力"
alt 客户端具备能力
RM->>C : "invokeClient(client : browser : invoke)"
C->>BM : "执行浏览器操作"
BM-->>C : "返回结果"
C-->>RM : "返回响应"
RM-->>S : "返回最终结果"
...
**图表来源**
- [packages/server-core/src/sessions/RemoteBrowserPaneManager.ts:72-94](file://packages/server-core/src/sessions/RemoteBrowserPaneManager.ts#L72-L94)
- [apps/electron/src/main/browser-pane-manager.ts:2386-2737](file://apps/electron/src/main/browser-pane-manager.ts#L2386-L2737)

### 本地实现与预加载
**新增** Electron 主进程实现了浏览器能力的具体处理：

- **预加载注册**：在应用启动时注册 client:browser:invoke 能力处理器
- **请求处理**：browser-pane-manager 接收并处理浏览器操作请求
- **错误处理**：对不支持的请求格式和未知方法抛出相应的 CodedError
- **方法执行**：调用真实的 BrowserPaneManager 实例执行浏览器操作
...
### 新增错误码定义
**增强** 传输层引入了更精细的错误代码处理机制：

- **BROWSER_NO_CAPABLE_CLIENT**：当没有连接的桌面客户端支持浏览器工具时抛出
- **CAPABILITY_UNAVAILABLE**：当客户端不声明或不支持特定能力时抛出
- **HANDLER_ERROR**：当能力处理器执行过程中发生错误时抛出
- **Unsupported browser capability request shape**：当请求格式不支持时抛出

### 错误处理流程
**增强** 错误处理流程更加完善：

- **能力验证错误**：在调用前进行能力检查，立即返回明确的错误信息
- **运行时错误**：在能力执行过程中捕获异常，转换为标准化的错误码
- **降级策略**：对于非关键错误，提供降级处理方案
- **日志记录**：详细的错误日志便于调试和问题定位

```mermaid
flowchart TD
A["能力调用请求"] --> B{"客户端具备能力?"}
B --> |否| C["抛出 CAPABILITY_UNAVAILABLE"]
B --> |是| D["验证请求格式"]
D --> |无效| E["抛出 HANDLER_ERROR"]
D --> |有效| F["执行能力处理器"]
F --> G{"执行成功?"}
G --> |否| H["抛出具体错误码"]
G --> |是| I["返回执行结果"]
...
**图表来源**
- [packages/server-core/src/sessions/RemoteBrowserPaneManager.ts:75-86](file://packages/server-core/src/sessions/RemoteBrowserPaneManager.ts#L75-L86)
- [apps/electron/src/main/browser-pane-manager.ts:2496-2737](file://apps/electron/src/main/browser-pane-manager.ts#L2496-L2737)

### 测试覆盖
**增强** 新增了全面的错误处理测试：

- **能力验证测试**：验证客户端能力检查逻辑
- **错误码测试**：测试各种错误情况下的错误码返回
- **降级处理测试**：验证错误降级和恢复机制
- **兼容性测试**：确保错误处理不影响现有功能

**章节来源**
- [packages/server-core/src/transport/__tests__/error-codes.test.ts:14-124](file://packages/server-core/src/transport/__tests__/error-codes.test.ts#L14-L124)
- [packages/server-core/src/sessions/RemoteBrowserPaneManager.ts:75-86](file://packages/server-core/src/sessions/RemoteBrowserPaneManager.ts#L75-L86)
- [apps/electron/src/main/browser-pane-manager.ts:2496-2737](file://apps/electron/src/main/browser-pane-manager.ts#L2496-L2737)
...
- **新增** 服务器核心依赖
  - server-core 提供能力系统和浏览器能力协议定义
  - Electron 主进程实现具体的能力处理器
...
Server --> Types
subgraph "服务器核心"
PCap["server-core/transport/capabilities.ts"]
PBCap["server-core/transport/browser-capability.ts"]
RBPM["server-core/sessions/RemoteBrowserPaneManager.ts"]
end
...
- **新增** 能力系统问题
  - 检查客户端是否在握手时正确声明了 clientCapabilities。
  - 验证服务器端是否正确识别和缓存了客户端能力信息。
  - 对于浏览器能力调用失败，检查客户端是否具备 client:browser:invoke 能力。
  - 查看具体的错误码以确定问题原因（如 BROWSER_NO_CAPABLE_CLIENT、CAPABILITY_UNAVAILABLE）。
...
## 结论
传输层通过统一的信封模型与严格的通道分类，实现了跨进程、跨工作区的稳定 RPC 通信。配合可靠的握手、心跳与重连机制，以及完善的编解码与安全策略，能够在复杂网络环境中保持高可用与高性能。

**新增的浏览器能力系统**进一步增强了传输层的功能性：
- 通过 client:browser:invoke 能力实现了远程浏览器操作
- 完整的能力声明、验证和路由机制确保了系统的可靠性
- 详细的错误处理和降级策略提高了系统的健壮性
...
- 对高频事件进行限速与批处理
- 建立完善的可观测性与告警体系
- **新增** 正确配置和使用能力系统，确保客户端能力声明的准确性

## 附录
- 关键类型与字段参考：MessageEnvelope、MessageType、WireError、ErrorCode
- 通道分类参考：LOCAL_ONLY_CHANNELS、REMOTE_ELIGIBLE（通过路由表与通道常量定义）
- **新增** 能力系统参考：CLIENT_BROWSER_INVOKE、LOCAL_CLIENT_CAPABILITIES
- **新增** 错误码参考：BROWSER_NO_CAPABLE_CLIENT、CAPABILITY_UNAVAILABLE、HANDLER_ERROR
...
- [packages/shared/src/protocol/channels.ts:412-448](file://packages/shared/src/protocol/channels.ts#L412-L448)
- [packages/server-core/src/transport/capabilities.ts:25-36](file://packages/server-core/src/transport/capabilities.ts#L25-L36)
- [packages/server-core/src/transport/browser-capability.ts:11-54](file://packages/server-core/src/transport/browser-capability.ts#L11-L54)